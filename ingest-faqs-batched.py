...
# Model to use for embeddings; you can switch to "text-embedding-ada-002" or another
EMBEDDING_MODEL = "text-embedding-ada-002"

# Max number of questions per OpenAI batch. 
# (You can increase up to ~2048 if you have a large CSV, but keep it small enough to avoid huge JSON bodies.)
BATCH_SIZE = 50

# Path to your trimmed CSV on the EC2
...
# Step 1: Read all rows from the CSV into memory
rows = []
...
# Step 2: Batch the "question" texts into chunks of size BATCH_SIZE for OpenAI
...
def get_embeddings(text_list):
    ...
    # We extract embeddings in the same order as input
...
# Step 3: Send each row+vector to Weaviate in one shot (no more OpenAI calls needed)
